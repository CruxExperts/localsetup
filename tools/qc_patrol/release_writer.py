...
def release_readiness(repo: Path, out: Path) -> dict[str, Any]:
    ...
    build = subprocess.run(["uv", "run", "--locked", "python", "ls/tools/localsetup.py", "--source-root", ".", "package", "--out", str(artifact)], cwd=repo, text=True, capture_output=True)
    findings = check_release_exclusions(repo, artifact if artifact.exists() else None)
    verify = None
    if build.returncode == 0:
        verify = subprocess.run(["uv", "run", "--locked", "python", "ls/tools/localsetup.py", "--source-root", ".", "verify-release", str(artifact)], cwd=repo, text=True, capture_output=True)
    return {
