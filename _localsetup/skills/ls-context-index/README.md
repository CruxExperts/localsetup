...
`mcp config` emits a deterministic optional MCP server configuration. MCP serving is intentionally not a hard dependency; `_localsetup/tools/context_mcp_server.py` is present as the stable wrapper target and reports a structured optional-dependency error until an MCP SDK implementation is enabled.
