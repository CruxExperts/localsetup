...
2. **Duplicate, overlap, and namespace check**  - List existing skills from `_localsetup/skills/` (dir names and each SKILL.md `name` + `description`). Compare the candidate: **Namespace collision** = same `name` or directory already exists; **High overlap** = very similar description/purpose/triggers. If either: warn and offer **Keep existing** (do nothing, skip import), **Replace existing** (overwrite with new), **Merge** (combine best of both into one skill), **Create as new** (use a different name, e.g. `ls-<name>-v2`). Get explicit user choice; do not auto-replace or auto-merge.
3. **Copy as-is**  - Place the approved candidate under `_localsetup/skills/<name>/` (or chosen name after Merge/Create as new). If adopting framework convention, use `ls-<name>` and set `name: ls-<name>` in frontmatter. Do not edit the body, references, scripts, or assets during this copy step.
