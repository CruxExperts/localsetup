...
- **External skills can be used here.** A skill from [anthropics/skills](https://github.com/anthropics/skills) or any spec-compliant source can be dropped into `_localsetup/skills/`, optionally renamed to `ls-*`, given `metadata.version` if missing, and registered; no body or structure changes needed for compliance.
...
3. **Copy**  - Place under `_localsetup/skills/<name>/` (or chosen name after Merge/Create as new). If keeping the original name, directory must match `name` in frontmatter. If adopting framework convention, use `ls-<name>` and set `name: ls-<name>` in frontmatter.
...
- _localsetup/docs/SKILL_DISCOVERY.md  - Public registries and discovery; use ls-skill-discovery when creating to recommend similar public skills.
