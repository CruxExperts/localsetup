...
- `detected_shape`: lockfiles, adapter paths, stale `_localsetup/`, protected source-root status, legacy global roots, and partial adapters.
...
- Do not delete `_localsetup/` unless it looks like copied Localsetup framework source and the active source root is elsewhere.
- Treat Localsetup maintainer/source checkouts as protected. If the target root contains source-root markers such as `_localsetup/config/pack.yaml`, `_localsetup/core/`, `VERSION`, and `pyproject.toml` or `install`, do not repair it as a consumer repo.
...
- Do not remove, replace, or convert any protected source checkout's `_localsetup/` tree through doctor repair.
