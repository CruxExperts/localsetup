...
Volatile facts below were verified on 2026-05-12 UTC from npm registry metadata
...
- `next` latest is `16.2.6`; `canary` is `16.3.0-canary.19`; `beta` is
...
- `react` and `react-dom` latest are `19.2.6`; `react-dom@19.2.6` peers on
  `react ^19.2.6`.
- Node `26.x` is Current; latest dist entry checked was `v26.1.0`.
- Node `24.x` Krypton is Active LTS; latest dist entry checked was `v24.15.0`.
- Node `22.x` Jod is Maintenance LTS; latest dist entry checked was `v22.22.2`.
