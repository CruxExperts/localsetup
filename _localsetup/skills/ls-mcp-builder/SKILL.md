...
- **Emulation**: No LLM, no API keys. A JSON file defines scripted tool calls and the expected response text per task. Use for smoke tests or when you do not want to configure APIs. Run with `--provider emulation --emulation-script assets/smoke_emulation.json` (script must have one task per qa_pair in the eval XML). Emulation fails the task on first tool error and reports it.

Smoke check (no connection): `python scripts/evaluation.py --help`. Full emulation run requires an MCP server and a matching emulation JSON (see `assets/smoke_emulation.json` and `assets/smoke_eval.xml`).
