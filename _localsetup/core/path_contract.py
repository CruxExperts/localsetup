...
def _ensure_source_root(source_root: Path) -> Path:
    ...
    if not framework.is_dir():
        raise PathValidationError(f"source-root must contain _localsetup: {root}")
    return root
...
def build_paths_manifest(source_root: Path, home: Path) -> dict[str, Any]:
    ...
    framework_root = source / "_localsetup"
...
def resolve_doc_path(source_root: Path, relative_path: str) -> Path:
    ...
    if rel.startswith("_localsetup/docs/"):
        rel = rel.removeprefix("_localsetup/docs/")
    return _ensure_source_root(source_root) / "_localsetup" / "docs" / rel


def resolve_tool_path(source_root: Path, relative_path: str) -> Path:
    rel = _safe_rel(relative_path, "tool path")
    if rel.startswith("_localsetup/tools/"):
        rel = rel.removeprefix("_localsetup/tools/")
    return _ensure_source_root(source_root) / "_localsetup" / "tools" / rel
