...
9. For long privileged maintenance, request bounded keepalive only after sudo is ready:
   ...
   ```bash
   ./_localsetup/tools/tmux_ops keepalive request -t ops --owner agent-id --ttl-seconds 7200 --max-refreshes 24 --reason "active privileged maintenance"
   ```

   Refreshes are one-shot calls, not a background loop:

   ```bash
   ./_localsetup/tools/tmux_ops keepalive refresh -t ops
   ```
...
12. If privileged work must keep the same sudo timestamp alive, request a bounded marker only after sudo is `ready`:
   ...
   ```bash
   ./_localsetup/tools/tmux_ops keepalive request -t <session> --owner <id> --ttl-seconds 7200 --max-refreshes 24 --reason "<reason>"
   ```
