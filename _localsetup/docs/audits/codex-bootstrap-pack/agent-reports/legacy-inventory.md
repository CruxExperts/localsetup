...
| `/mnt/data/devzone/localsetup/.agents/skills` | Legacy duplicate candidate | 50 skill dirs, older `localsetup-*` names, sample hashes differ |
...
Any write, delete, rename, symlink, permission, or package-install action outside the current repo requires explicit user approval. That includes `/mnt/data/devzone/localsetup`, `~/.codex`, `~/.agents`, `~/.cursor`, `~/.config/kilo`, `~/.config/opencode`, and `~/.local/share/agents/skills/localsetup`.
...
- Migrate `~/.codex/skills` and `/mnt/data/devzone/localsetup/.agents/skills` only after approval.
