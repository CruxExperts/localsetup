...
- **Tmux shared session and sudo:** Load ls-tmux-shared-session-workflow; use tool `_localsetup/tools/tmux_ops` (`pick`, `probe`, `run`, `status`, `cancel`). Skill defines sudo gate via probe (`ready`, `password_required`, or `failed`); run commands with `run -t SESSION -- CMD`, inspect returned `tail`/`log_path`, watch active runs with `status --wait`, and interrupt only with `cancel --run-id`. For remote/VMs: see [ops/tmux-ops-remote.md](ops/tmux-ops-remote.md) (REMOTE_TMUX_HOST). See WORKFLOW_REGISTRY.md.
