...
from _localsetup.core.path_contract import paths_manifest_issues, paths_manifest_path
from _localsetup.core.path_reprocessor import reprocess_localsetup_paths
from _localsetup.core.repair import run_repair
from _localsetup.core.test_workers import default_test_workers, resolved_test_workers
from _localsetup.tests.test_install_flow import make_temp_repo
...
def test_localsetup_path_cli_writes_json_manifest_and_resolves_paths(tmp_path: Path) -> None:
    ...
    assert payload["paths"]["docs-root"] == str(root / "_localsetup" / "docs")
    assert payload["paths"]["tools-root"] == str(root / "_localsetup" / "tools")
    ...
    assert doc.stdout.strip() == str(root / "_localsetup" / "docs" / "ops" / "tmux-ops-managed.md")
...
def test_full_pytest_procedure_surfaces_use_hardened_worker_helper() -> None:
    surfaces = [
        ...
        SOURCE_ROOT / "_localsetup" / "README.md",
        SOURCE_ROOT / "_localsetup" / "core" / "context.py",
        SOURCE_ROOT / "_localsetup" / "docs" / "COMMAND_REFERENCE.md",
        SOURCE_ROOT / "_localsetup" / "docs" / "REPO_MAINTENANCE.md",
        SOURCE_ROOT / "_localsetup" / "skills" / "ls-context" / "SKILL.md",
        SOURCE_ROOT / "_localsetup" / "skills" / "ls-framework-compliance" / "SKILL.md",
        SOURCE_ROOT / "_localsetup" / "templates" / "codex" / "AGENTS.md",
    ]
    ...
    assert "test-workers" in (SOURCE_ROOT / "_localsetup" / "core" / "context.py").read_text(encoding="utf-8")
