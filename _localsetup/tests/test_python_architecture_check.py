...
CHECKER = ROOT / "_localsetup" / "tools" / "python_architecture_check.py"
POINTER = (
...
def make_repo(tmp_path: Path, *, large_lines: int = 10) -> Path:
    repo = tmp_path / "repo"
    repo.mkdir()
    write(repo / "_localsetup/core/demo.py", "\n".join("x = 1" for _ in range(large_lines)) + "\n")
    write(repo / "_localsetup/tools/python_architecture_check.py", "# wrapper\n")
    write(
        repo / "_localsetup/docs/PYTHON_ARCHITECTURE_STANDARD.md",
    ...
    write(
        repo / "_localsetup/config/python-architecture-baseline.json",
        json.dumps(
            {
                "schema_version": "1.0",
...
def run_checker(repo: Path, *args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            ...
            str(repo),
            "--baseline",
            "_localsetup/config/python-architecture-baseline.json",
            *args,
        ],
        cwd=ROOT,
        check=False,
...
def test_checker_stale_baseline_entry_warns(tmp_path: Path) -> None:
    ...
    baseline = {
        ...
        "entries": [
            {
                "id": "missing",
                "path": "_localsetup/core/missing.py",
                "metric": "lines",
                "current_value": 701,
                "threshold": 700,
...
def test_checker_missing_doc_anchor_fails(tmp_path: Path) -> None:
    repo = make_repo(tmp_path)
    text = (repo / "_localsetup/docs/PYTHON_ARCHITECTURE_STANDARD.md").read_text(encoding="utf-8")
    write(repo / "_localsetup/docs/PYTHON_ARCHITECTURE_STANDARD.md", text.replace("# Source Evidence", ""))
...
def test_checker_missing_template_pointer_fails(tmp_path: Path) -> None:
    repo = make_repo(tmp_path)
    write(repo / "_localsetup/templates/codex/AGENTS.md", "missing pointer\n")
...
def test_checker_missing_wrapper_fails(tmp_path: Path) -> None:
    repo = make_repo(tmp_path)
    (repo / "_localsetup/tools/python_architecture_check.py").unlink()
...
def test_template_parity_uses_pointer_and_required_anchors() -> None:
    standard = (ROOT / "_localsetup/docs/PYTHON_ARCHITECTURE_STANDARD.md").read_text(encoding="utf-8")
