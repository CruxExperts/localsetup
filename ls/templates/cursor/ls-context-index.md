...
| ls-omniroute-proxy | All read-only OmniRoute model/provider, context, observability, integration, client and endpoint discovery, plus sanitized model observations | OmniRoute catalogs, diagnostics, integrations, context state, client compatibility, or normalized observations |
| ls-omniroute-admin-automation | All OmniRoute writes, imports, purges, services, settings, providers, keys, integrations, backup/restore, and rollback-safe reconciliation | Any OmniRoute mutation, import, service action, reconciliation, backup/restore, key or policy management |
