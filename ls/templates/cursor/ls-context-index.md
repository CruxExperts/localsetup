...
| ls-omniroute | Main OmniRoute router for issue triage, env/API-key preflight, access compatibility checks, and choosing the focused OmniRoute skill | Any OmniRoute task, especially first-response troubleshooting or deciding which OmniRoute skill to load |
| ls-omniroute-proxy | OmniRoute runtime discovery, model catalogs, provider metadata, limits, quotas, routing combos, MCP/A2A discovery, and agent client configuration | OmniRoute catalogs, provider limits, routing combos, or configuring agents to use OmniRoute |
...
| ls-omniroute-observability | OmniRoute health, usage, quota, cost, policy, audit, resilience, and evaluation diagnostics | Health checks, quota/cost reports, logs, audit trails, resilience state, or benchmark status |
| ls-omniroute-context | OmniRoute context engineering, compression, memory, cache, RTK, and Qdrant configuration workflows | Prompt compression, memory/cache, RTK filters, context relay, or Qdrant settings |
| ls-omniroute-integrations | OmniRoute MCP, A2A, CLI tools, plugins, skills, tunnels, webhooks, and external integration diagnostics | Protocol integrations, tool/plugin surfaces, tunnels, webhooks, or agent bridge diagnostics |
| ls-omniroute-codex | OmniRoute Codex CLI onboarding, setup/launch guidance, profile files, and `wire_api = "responses"` configuration | Configure Codex or troubleshoot Codex profile/API routing through OmniRoute |
| ls-omniroute-update | OmniRoute update reporting for upstream skill discovery, Localsetup coverage comparison, provenance metadata, and report-first maintenance planning | Listing, checking, updating, modifying, importing, or removing OmniRoute-derived Localsetup skills |
...
Framework docs: ls/docs/ (AGENTIC_DESIGN_INDEX.md, WORKFLOW_REGISTRY.md, PRD_SCHEMA_EXTERNAL_AGENT_GUIDE.md).
