...
def provision(root: Path, runtime_root: Path, assets_dir: Path, python: str) -> dict:
    ...
    for name in ("sdk-build.lock", "sdk-runtime.lock"):
        subprocess.run([python, "-m", "pip", "download", "--disable-pip-version-check",
                        "--require-hashes", "--no-deps", "--no-build-isolation",
                        "--dest", str(wheelhouse), "-r", str(assets_dir / name)],
                       cwd=assets_dir, check=True, timeout=300, stdout=subprocess.DEVNULL,
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except (ValueError, OSError, RuntimeError, subprocess.SubprocessError, KeyError):
        print(json.dumps({"ok": False, "reason": "Protected runtime provisioning failed; inspect published wheel, hashed exports and retained task paths"}))
        return 1
