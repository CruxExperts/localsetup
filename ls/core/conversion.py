...
def _framework_sync_needed(source_root: Path, target_root: Path) -> bool:
    source_framework = (source_root / "ls").resolve(strict=False)
    target_framework = (target_root / "ls").resolve(strict=False)
...
def _inventory(
    ...
    *,
    ...
    target_framework = target_root / "ls"
...
def convert_repo(
    ...
    *,
    ...
    payload["framework_source"] = {
        "copied": False,
        "source_root": str(source_root),
        "target_framework_absent": not (target / "ls").exists(),
    }
    pack = load_pack_config(source_root)
    dependency_info = (
        ensure_dependencies(
