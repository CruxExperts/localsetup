...
1. **Session.** Run `./ls/tools/tmux_ops pick`. Use the returned
...
2. **Gate.** Run `./ls/tools/tmux_ops probe -t <session>`. If the
...
3. **Run.** One logical step per managed run:
   `./ls/tools/tmux_ops run -t <session> -- <cmd>`. Read the returned
