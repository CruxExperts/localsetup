...
from .repair_safety import ls_owned_adapter_dir, _symlink_target_under_managed_roots
...
def _plan_actions(
    ...
    *,
    ...
    stale_framework = target_root / "ls"
        ...
        if stale_class == "protected_source_root":
            ...
            actions.append(
                _action(
                    ...
                    stale_framework,
                    safety="safe",
                    reason="clean tracked consumer ls framework source can be untracked before removal",
            ...
            actions.append(
                _action(
                    ...
                    stale_framework,
                    safety="safe",
                    reason="consumer repo should not keep copied ls framework source",
                )
            )
            pre_action_count += 1
            ...
            decisions.append(
                {
                    ...
                    "reason": "framework-like ls has Git changes or mixed tracked/untracked state",
                    "required": "review migration plan before removal",
                    "prompt_hint": "Inspect Git state under ls and decide whether content is user-owned.",
                }
            )
        elif stale_class == "custom_framework_content":
            decisions.append(
                {
                    "kind": "stale_framework",
            ...
            decisions.append(
                {
                    ...
                    "path": str(stale_framework),
                    "reason": "ls is not a supported directory node",
            ...
            if tracked and "tracked-framework-removal" in allow:
                actions.append(
                    _action(
                        ...
                        stale_framework,
                        safety="safe",
                        reason="tracked consumer ls framework source removal explicitly allowed",
                    )
                )
                actions.append(
                    _action(
    ...
    for target in adapter_targets(source_root, home, platform_ids=platform_ids, target_root=target_root):
        ...
        if reason in {"dangling symlink"}:
            ...
            if ls_owned_adapter_dir(source_root, path, decisions):
                actions.append(
                    _action(
    ...
    for platform_id, rel_paths in HISTORICAL_ADAPTERS.items():
        ...
        for rel in rel_paths:
            ...
            if state["status_code"] in {"custom_repo_skills", "shared_adapter_directory", "mixed_managed_custom_adapter"} and (
                ...
                continue
            if (path.is_symlink() and _symlink_target_under_managed_roots(path, managed_roots)) or ls_owned_adapter_dir(source_root, path, decisions):
                actions.append(
                    _action(
...
def _apply_pre_actions(actions: list[dict], backup_root: Path, target_root: Path, global_root: Path, known_roots: list[Path]) -> list[str]:
    ...
    for action in actions:
        ...
        if action["kind"] == "remove_managed_adapter_entries":
            ...
            continue
        backups.append(_backup_item(path, backup_root, target_root))
        if action["kind"] == "backup_remove_stale_framework" and path.name == "ls":
            git_state = inspect_path(target_root, "ls")
            if git_state.get("tracked_entries"):
                result = git_untrack_path(target_root, "ls")
                if not result["ok"]:
                    raise RuntimeError(f"failed to untrack ls before removal: {result['stderr'] or result['stdout']}")
        _remove_path(path)
    return backups
