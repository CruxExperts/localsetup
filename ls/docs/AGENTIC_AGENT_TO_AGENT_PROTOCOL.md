...
**Build contract:** [AGENTIC_AGENT_Q_BIDIRECTIONAL_BUILD_SPEC.md](AGENTIC_AGENT_Q_BIDIRECTIONAL_BUILD_SPEC.md) (previous.0) is the implementation order; core pipeline and file_drop ingest are implemented under `ls/tools/agentq_transport_client/`. Mail adapter post-ingest move remains policy-gated via mail skill.
