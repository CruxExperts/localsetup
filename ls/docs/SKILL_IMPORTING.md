...
2. **Run validation on that path**  - From the Localsetup source checkout, run the scan tool against the temporary parent directory with `ls/tools/skill_importer_scan <temp-parent>`. If you need content-safety references for one specific candidate, run `ls/tools/skill_validation_scan.py --scan-root <temp-parent> <tempdir>`. Validation is always path-based; do not pass skill content through the shell or on stdin.
...
- [SKILL_NORMALIZATION.md](SKILL_NORMALIZATION.md)  - Phase 1 document and Phase 2 tooling normalization applied in staging after the vetting gate passes; includes platform-specific choices, approval, and the keep-original-tooling exception.
