...
The archive is a gzip-compressed POSIX/PAX tar stream with exactly one top-level directory. Its top-level name is the source directory basename and must match the safe-name rule `[A-Za-z0-9._-]+`, excluding `.` and `..`. The archive preserves regular files, directories, and permission bits. A top-level `.git` entry must be a directory; Gitfiles such as linked-worktree or submodule pointers are rejected because they reference external Git metadata and cannot form a self-contained archive. Symbolic links and hard links are rejected, along with FIFOs, sockets, device nodes, and other non-portable special filesystem entries, before the archive is committed. The output archive must be outside the source tree.
