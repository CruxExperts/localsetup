...
def resolve_pattern_file_path(pattern_file: Path | None, scan_root: Path) -> Path:
    """Resolve path: explicit > ls/docs/ (under scan_root) > ls/docs/."""
    if pattern_file:
        p = Path(pattern_file)
        if p.is_absolute():
            return p
        return (scan_root / p).resolve()
    # Default: ls/docs/ when running from client repo
    localsetup = scan_root / "ls" / "docs" / "SKILL_VALIDATION_PATTERNS.yaml"
    if localsetup.exists():
        return localsetup
    # Fallback: framework from source (script lives in ls/tools/)
...
def main() -> int:
    ...
    ap.add_argument("--scan-root", type=Path, default=None, help="Root path for resolving ls/docs (default: parent of skill_dir or cwd)")
