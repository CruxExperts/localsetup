...
from ls.core.agent.process_rpc import METHODS
...
async def main():
    ...
    async def stream(messages,info):
        ...
        if turns==1:
            ...
            yield {0:DeltaToolCall(name='run_command',json_args=json.dumps({'name':'test'}),tool_call_id='test-1')}
        else:
            returned=[part for message in messages for part in message.parts if isinstance(part,ToolReturnPart)][-1].content
            assert returned['status']=='completed' and returned['output']['stdout']=='fixture passed\n'
            yield 'file updated and tested'
    ...
    result=await iterate(FunctionModel(stream_function=stream),finder,prompt='Read and update fixture.',instructions='Use only granted files.',
                         tools=(*file_tools(finder,channel),process_tool(finder,channel)),store=store,on_event=emit,check=lambda:None,
