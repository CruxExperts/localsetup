"""Isolated compaction fixture with deterministic Chat Completions transport."""
...
async def main():
    ...
    def receive(request):
        ...
        assert body['stream'] and not body.get('tools') and 0<body['max_completion_tokens']<=4096
    ...
    profile=parse({'base_url':'https://fixture.invalid/v1/','api':'chat_completions','model':'fixture','credential_env':'KEY','timeout_seconds':5,'capabilities':['streaming'],'allow_loopback_http':False})
    ...
    async with model(profile,{'KEY':'fixture'},finder,transport=httpx.MockTransport(receive)) as adapter:
        ...
        for failure_mode,limit in [('oversize',10000),('tokens',5),('error',10000)]:
            mode=failure_mode
            before=len(calls)
    ...
    print(json.dumps({'requests':1,'failure_requests':3,'user_agent':calls[0].headers['user-agent'],'native_tail_preserved':True,'summary_is_user_context':True}))
asyncio.run(main())
