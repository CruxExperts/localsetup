...
from ls.core.repo_finalizer import plan as finalizer_plan
from ls.core.repo_finalizer import run as finalizer_run
from ls.core.repo_finalizer import status as finalizer_status
...
def test_allowlisted_managed_output_stages(tmp_path: Path) -> None:
    repo = _init_repo(tmp_path)
    target = repo / "ls" / "docs" / "_generated"
    ...
    assert payload["ok"] is True
    assert _class_for(payload, "ls/docs/_generated/facts.json") == "generated_artifact"
    staged = _git(repo, "diff", "--cached", "--name-only")
    assert "ls/docs/_generated/facts.json" in staged.stdout
...
def test_unknown_modified_block(tmp_path: Path) -> None:
    ...
    assert _class_for(payload, "ls/config/generated.txt") == "stale_legacy_framework_source"
...
def test_checkpoint_commit_only_allowlisted_files(tmp_path: Path) -> None:
    repo = _init_repo(tmp_path)
    managed = repo / "ls" / "docs" / "_generated"
    managed.mkdir(parents=True)
    ...
    assert changed == ["ls/docs/_generated/facts.json"]
...
def test_deleted_allowlisted_file_blocks(tmp_path: Path) -> None:
    ...
    facts.write_text("{}\n", encoding="utf-8")
    _git(repo, "add", "--", "ls/docs/_generated/facts.json")
    ...
    assert payload["ok"] is False
    row = next(row for row in payload["files"] if row["path"] == "ls/docs/_generated/facts.json")
...
def test_renamed_file_into_allowlist_blocks(tmp_path: Path) -> None:
    ...
    generated = repo / "ls" / "docs" / "_generated"
    generated.mkdir(parents=True)
    source.rename(generated / "facts.json")
    _git(repo, "add", "--", "notes.md", "ls/docs/_generated/facts.json")
