...
def test_cline_variants_share_paths_and_preserve_owners_and_native_state(tmp_path, mode):
    root = make_temp_repo(tmp_path);home = tmp_path / 'home'
    clients = ['cline-cli', 'cline-vscode']
    for base in (root, home):
        path = base / '.cline/skills';path.mkdir(parents=True)
        (path / 'custom.txt').write_text('keep')
    native = home / '.cline/data/settings/settings.json';native.parent.mkdir(parents=True)
    native.write_text('{"fixture": true}')
    session = home / '.cline/data/sessions/fixture';session.parent.mkdir(parents=True);session.write_text('keep session')
    ...
    assert (root / '.cline/skills/ls-context/SKILL.md').is_file()
    assert verify_install(root, home, target_root=root)['ok']
    assert detach_personal(root, home, [clients[0]], apply=True)['applied']
    assert (home / '.cline/skills/ls-context/SKILL.md').is_file()
    lock = json.loads(receipt.read_text())
    assert lock['platforms'] == [clients[1]]
    assert verify_install(root, home, target_root=root)['ok']
    for base in (root, home):assert (base / '.cline/skills/custom.txt').read_text() == 'keep'
    assert native.read_text() == '{"fixture": true}'
    assert session.read_text() == 'keep session'


def test_cline_declarations_separate_host_and_filesystem_qualification():
    registry = load_client_registry(Path(__file__).resolve().parents[2])
    for variant_id, kind in [('cline-cli', 'cli'), ('cline-vscode', 'ide')]:
        row = registry.variant('cline', variant_id).data
