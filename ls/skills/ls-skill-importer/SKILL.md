...
4. **Check duplicates and overlap**  - Compare selected candidates with `ls/skills/` by directory name, frontmatter `name`, description, purpose, and triggers. On collision or high overlap, offer **Ignore new**, **Replace existing**, **Merge**, or **Create as new**; get explicit user choice.
...
6. **Import and confirm**  - Copy the normalized skill to `ls/skills/<name>/`, align frontmatter `name` with the directory, add `metadata.version: "1.0"` if missing, register per [PLATFORM_REGISTRY.md](../../docs/PLATFORM_REGISTRY.md), and report what changed.
