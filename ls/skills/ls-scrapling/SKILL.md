...
This skill owns the Scrapling cheat-sheet behavior. `ls/docs/scrapling-cheat-sheet.md` is the public quick reference, but install/upgrade, host-vs-Docker choice, adapter refresh, helper verb signatures, and status artifact rules live here.
...
Agents should typically work through the public helper functions in `ls.tools.scrapling_helper.main`:
...
- Host-first:
  ...
  - Follow the shared CLI skills environment policy in `ls/docs/CLI_SKILLS_ENV.md` for pipx usage, PATH handling, and health checks.
