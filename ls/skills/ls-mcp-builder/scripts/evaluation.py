...
Examples:
  ...
  python evaluation.py --provider emulation --emulation-script emulation.json -t stdio -c python -a my_server.py eval.xml
    ...
    parser.add_argument("-t", "--transport", choices=["stdio", "sse", "streamable-http"], default="stdio", help="MCP transport (default: stdio)")
    stdio_group = parser.add_argument_group("stdio options")
    stdio_group.add_argument("-c", "--command", help="Command to run MCP server (stdio only)")
    stdio_group.add_argument("-a", "--args", nargs="+", help="Arguments for the command (stdio only)")
    stdio_group.add_argument("-e", "--env", nargs="+", help="Environment variables in KEY=VALUE format (stdio only)")
    remote_group = parser.add_argument_group("remote transport options")
    remote_group.add_argument("-u", "--url", help="MCP server URL (sse/streamable-http only)")
    remote_group.add_argument("-H", "--header", nargs="+", dest="headers", help="HTTP headers in 'Key: Value' format (sse/streamable-http only)")
