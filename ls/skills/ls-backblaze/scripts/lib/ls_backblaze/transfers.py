...
def _remote_parts(client: Any, args: dict[str, Any], state: dict[str, Any]) -> dict[int, dict[str, Any]]:
    ...
    for _ in range(10001):
        request = {"Bucket": args["bucket"], "Key": args["key"], "UploadId": state["upload_id"]}
...
def _upload(client: Any, args: dict[str, Any], account_marker: str, *, resume: bool = False) -> dict[str, Any]:
    ...
    binding = {"provider": "backblaze-b2", "account": account_marker,
    ...
    state.update(phase="completed", result={"version_id": response.get("VersionId"), "etag": response["ETag"]})
    ...
    try:
        ...
        result.update(partial=True, reconciliation="completion succeeded but checkpoint update failed; inspect the reported version")
    return result
