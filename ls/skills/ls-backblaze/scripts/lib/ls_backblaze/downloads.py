...
def download(client: Any, args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"destination": str(destination), "backup": str(backup) if backup else None, "content_length": size, "version_id": response.get("VersionId"), "checksum": checksum, "checksum_verified": verified, "sha256": digest.hexdigest(), "assurance": "verified length and provider SHA256" if verified else "verified length; locally computed SHA256; provider checksum unverified or absent"}
    except ToolError:
        raise
