...
(e.g. python3 scripts/pr_review.py --help). Follows INPUT_HARDENING_STANDARD and TOOLING_POLICY.
...
def _framework_lib_path() -> Path | None:
    candidate = Path(__file__).resolve().parents[3] / "lib"
    return candidate if (candidate / "deps.py").is_file() else None


def _smoke_env() -> dict[str, str]:
    env = os.environ.copy()
    lib_path = _framework_lib_path()
    if lib_path is None:
        return env
...
def _sanitize_command(s: str) -> list[str]:
