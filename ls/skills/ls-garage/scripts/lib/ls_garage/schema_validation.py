...
def validate(value: Any, schema: dict[str, Any], path: str = "args", root: dict[str, Any] | None = None) -> None:
    ...
    if choices and not any(type(value) is types[item] for item in choices):
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if "propertyNames" in schema:
                validate(key, schema["propertyNames"], path + ".<name>", root)
            if key in properties:
                validate(item, properties[key], f"{path}.{key}", root)
            elif isinstance(additional, dict):
                validate(item, additional, f"{path}.{key}", root)
